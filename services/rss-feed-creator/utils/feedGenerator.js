...
const FEED_RETENTION_DAYS = Number(ENV.rss.FEED_RETENTION_DAYS) || 7; // default 7 days
...
export async function generateFeed(bucket, rewrittenItems) {
  try {
    ...
    const channelTitle =
      ENV.rss.TITLE?.trim() || "Turing's Torch: AI Weekly";
    const channelLink = "https://jonathan-harris.online";
    const channelDesc =
      ENV.rss.DESCRIPTION?.trim() ||
    ...
    await uploadBuffer(bucket, "feed.xml", xmlString, "application/rss+xml");
    await uploadBuffer(bucket, "feed.json", jsonString, "application/json");
...
async function loadExistingFeedItems(bucket) {
  try {
    const jsonContent = await getObjectAsText(bucket, "feed.json");
